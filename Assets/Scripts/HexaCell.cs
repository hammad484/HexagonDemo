...
public class HexaCell : CellManager {
	GridUpdater GridManagerObject;
	public int x;
	public int y;
	public Color color;
	...
	void Update() {
		if (lerp) {
			float newX = Mathf.Lerp(transform.position.x, lerpPosition.x, Time.deltaTime*Cell_rotate);
			float newY = Mathf.Lerp(transform.position.y, lerpPosition.y, Time.deltaTime*Cell_rotate);
			transform.position = new Vector2(newX, newY);

			
			if (Vector3.Distance(transform.position, lerpPosition) < CellRotationThres) {
